...
export function ExperienceCard({ entry, delay = 0 }: { entry: Experience; delay?: number }) {
  ...
  return (
    <motion.div
      ...
      className="hud-corners relative bg-surface border border-border-soft rounded-2xl overflow-hidden flex flex-col sm:flex-row transition-[border-color,box-shadow] duration-300 hover:border-signal/40 hover:glow-signal"
    >
      <HudCorners />
      {entry.image && (
        <div
      ...
      <div className="p-6 sm:p-7 flex flex-col flex-1 justify-center">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {entry.stack.map((tag, i) => (
            <span
              key={i}
              className="text-xs font-medium text-signal bg-signal-dim px-2.5 py-0.5 rounded-md"
            >
              {tagLabel(tag, lang)}
            </span>
          ))}
